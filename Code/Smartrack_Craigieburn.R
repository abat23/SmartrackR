...
express <- c("Flemington Racecourse","EXP Flag","Broadmeadows")
ltd_express <- c("Flemington Racecourse","Essendon","Glenberive","Strathmore","Pascoe Vale","Oak Park","Glenroy","Broadmeadows")
sas <- c("Newmarket","Ascot Vale","Moonee Ponds","Essendon")
...
while(stops <= length(buses$origin)) {
  ...
  (
    ...
    (lead(org,2)[stops] == express[1] && lead(dest,1)[stops] == first(express))
  ))
  {
  ...
  #LTD EXPRESS BUS
    ...
    (lead(org,5)[stops] == ltd_express[6] && lead(dest,5)[stops] == ltd_express[7]) &&
     (lead(org,6)[stops] == ltd_express[7] && lead(dest,6)[stops] == last(ltd_express))
  )
  ||
  ( #lasst = 7
    ...
    (lead(org,4)[stops] == ltd_express[6] && lead(dest,4)[stops] == ltd_express[3]) &&
  ...
  if ((
    ...
    (lead(org,2)[stops] == sas[3] && lead(dest,2)[stops] == last(sas))
  )
  ||
  (
    (org[stops] == last(sas) && dest[stops] == sas[3]) &&
    # (lead(org,1)[stops] == sas[4] && lead(dest,1)[stops] == sas[3]) &&
    (lead(org,1)[stops] == sas[3] && lead(dest,1)[stops] == sas[3]) &&
    (lead(org,2)[stops] == sas[2] && lead(dest,3)[stops] == first(sas))
  ))
  {
...
write.csv(travel_times,paste0("CTD Bus Leg Times.csv"))
